...
> 文档版本：0.2.0 · 修订日期：2026-09-10
> 状态：进入原型验证前的设计与开发交付稿；不是已完成产品。
> 本版取代早期“Readiness Score / 迁移率 / 保证安全删除”的方案。命令、接口和示例输出均为拟议契约，不表示已经实现。
...
以下命令需要开发后才能运行。示例 `example-lib` 及路径为虚构测试对象。
...
名称 SunsetGuard 暂为工作名，未确认 npm 注册或其他权利状态。开源许可证建议在正式发布前选择并确认；Codex 不应虚构版权主体。外部源码样本应保留来源、许可和必要归属，尽量使用原创最小 fixtures，不把公开可读等同于可任意再分发。
