...
2026-09-11：完成首批真实文件子集扫描、标签与评估。**所有标签仍为 provisional，由初始评估者在查看预测前阅读源码建立，未经独立人工复核。** 这不是全仓库扫描、外部精度认证或维护者试用证据。
...
三个根 LICENSE 均为 MIT，随外部样本保存并记录摘要。源码及其注释中的命令只作为数据，不复制到本工程、不执行、不安装下游依赖。准备脚本将样本写入项目目录外的新目录，避免与工具源码和测试发现范围混合。

## 标签与计数

`annotations.json` 在查看本轮预测前冻结，记录理由、一基 UTF-16 位置和 binding 位置。每个文件在初始标注阶段均全文阅读；`exhaustive=true` 只表示对这个文件、这个目标的预期 finding 集合穷尽，不等于独立人审。没有从 AST 预测复制标签。
...
该 benchmark 的工程流程已完成；独立人工复核、维护者试用和其他语法模式的真实精度仍未验证。固定 GitHub commit 获取和 Linux/Windows CI 已在后续版本实现，但不会改变这组标签的 provisional 状态。
